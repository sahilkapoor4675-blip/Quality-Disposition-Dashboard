#!/usr/bin/env python3
"""Non-destructive HTTP regression suite.
...
"""
import json, os, shutil, sqlite3, subprocess, sys, tempfile, time, urllib.parse, urllib.request
...
def main():
    tmpdir = tempfile.mkdtemp(prefix="qcr_regression_")
    db = os.path.join(tmpdir, "quality.db")
    shutil.copy2(ROOT/"quality.db", db)
    try:
        ...
        env["DB_PATH"]=db  # isolated, disposable — never the real persistent database
        ...
        try:
            ...
            print(f"REGRESSION PASS — {before} disposition records unchanged; {len(checks)} core endpoints + filter contract OK.")
        finally:
            p.terminate();
